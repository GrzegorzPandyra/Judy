#ifndef CONFIG_H
#define CONFIG_H

#define PROJECT_ROOT "C:\\Users\\Greg\\Dropbox\\PROGRAMOWANIE\\C\\Judy"
#define LOG_PATH     "C:\\Users\\Greg\\Dropbox\\PROGRAMOWANIE\\C\\Judy\\logs\\debug.log"

#define SCREEN_WIDTH  121
#define SCREEN_HEIGHT  53
#define SCREEN_X  0
#define SCREEN_Y  0
#define SCREEN_WRAP_DISTANCE 20
#define BUFFER_WIDTH  121
#define BUFFER_HEIGHT  1000
#define INFO_WINDOW_HEIGHT 3
#define NAVIGATION_PANEL_HEIGHT 4
#define FILEPATH_X  6
#define FILEPATH_Y  1

#define HORIZONTAL_BORDER_CHAR  '-'
#define VERTICAL_BORDER_CHAR  '|'

#define PROGRAM_NAME_X 54
#define PROGRAM_NAME_Y 0
#define PROGRAM_NAME "Judy"
#define PROGRAM_VERSION "4"
#define PADDING 5

#define INTERFACE_MAX_PATH_LENGTH 130
#define PASSWORD_MAX_LENGTH 50
#define DEBUG_MAX_STRING_LENGTH 150

#define PAGING_DISTANCE 25
#define LINE_NUM_OFFSET 7

#define TAB 9 
#define SPACE 32 
#define ENDL "\n"

#define MATRIX_SIZE 94							// '!' - '~'
#define MATRIX_START_CHAR 33					// '!'

#define ENCRYPT_BTN_1 'e'
#define ENCRYPT_BTN_2 'E'
#define DECRYPT_BTN_1 'd'
#define DECRYPT_BTN_2 'D'
#define PAGE_UP_BTN_1 'p'
#define PAGE_UP_BTN_2 'P'
#define PAGE_DOWN_BTN_1 'l'
#define PAGE_DOWN_BTN_2 'L'
#define SCROLL_UP_BTN_1 'w'
#define SCROLL_UP_BTN_2 'W'
#define SCROLL_DOWN_BTN_1 's'
#define SCROLL_DOWN_BTN_2 'S'
#define OPEN_FILE_BTN_1 'o'
#define OPEN_FILE_BTN_2 'O'
#define CLOSE_FILE_BTN_1 'c'
#define CLOSE_FILE_BTN_2 'C'
#define SAVE_AS_BTN_1 't'
#define SAVE_AS_BTN_2 'T'
#define QUIT_BTN_1 'q'
#define QUIT_BTN_2 'Q'
#endif